...
\begin{frame}
  ...
  \only<5->{See also \citet{pearl:2000a} and also
	  \cite{robins2010alternative}. For more on the potential outcomes
...
\begin{frame}
	\frametitle{What do we need to assume for statistical inference about
		counterfactual causal quantities?}
