...
\section{Why test a hypothesis? What is a hypothesis?}
...
\begin{frame}
	...
	The false positive rates of permutation based tests are controlled
	for both small and large sample sizes.

\centering
\only<2>{\includegraphics[width=.95\textwidth]{newspapersTDist.pdf} }
\only<3>{\includegraphics[width=.95\textwidth]{newspapersTypeIError.pdf} }
