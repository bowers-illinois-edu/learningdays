...
\begin{frame}{Inferring about Causal Effects}

How can we use what we \textbf{see} to learn about
\only<1>{what we want to \textbf{know}} \only<3>{$\text{a causal effect}_i=f(y_{i,1},y_{i,0})$} \only<2>{\textbf{\textcolor{blue}{potential outcomes}}}?
...
\only<3>{\textbf{\emph{Small group exercise:}} \textbf{Can you think of \textcolor{orange}{more than one way} to learn about the counterfactual causal effect of treatment using what we observe?}}
\end{frame}

\section{Definition of a p-value. What does it mean?}
...
# Testing Fishers Sharp Null Hypothesis of No Effects: Where do $p$-values come from?
...
```{r sharpnulltest1,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
...
```{r sharpnulltest1ps,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
...
# Testing Sharp Hypotheses No Effects and No Interference
...
```{r lmps,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
...
# What is a $p$-value? What does a $p$-value require?
...
```{r sharpnulltest1exact,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
...
```{r sharpnulltest2,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
table(mdist)
table(rdist)
...
```{r installdevritools, eval=FALSE,echo=FALSE}
## Only run this once
...
library(devtools)
install_github("markmfredrickson/RItools@randomization-distribution",force=TRUE,lib="libraries")
```

```{r sharpnulltest3,echo=TRUE,results='markup',highlight=TRUE,tidy=FALSE,tidy.opts=list(width.cutoff=130)}
