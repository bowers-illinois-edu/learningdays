...
dat$complete.ra <- complete.ra

dat$complete.obs <- with(dat,Y1*complete.ra+Y0*(1-complete.ra))
...
av.treat <- mean(dat$complete.obs[dat$complete.ra==1])
av.control <- mean(dat$complete.obs[dat$complete.ra==0])
...
treated <- dat$complete.obs[dat$complete.ra==1]
treated
...
not_treated <- dat$complete.obs[dat$complete.ra==0]
not_treated
...
lm(complete.obs~complete.ra, data=dat)
...
for (i in 1:10000){
	perm_matrix[i,] <- sample(dat$complete.ra, 600, replace=F)
}
...
randmeandiff <- NA # placeholder vector for results
for (i in 1:nrow(perm_matrix)){ # for each one of the "fake" treatment vectors

  mean_treat <- mean(dat$complete.obs[perm_matrix[i,]==1])

  mean_control <- mean(dat$complete.obs[perm_matrix[i,]==0])

  # calculating difference of means for this randomization
  randmeandiff[i] <- mean_treat - mean_control

}

summary(randmeandiff) # vector of permutation of differences.

# We can make a plot to better see the results:

hist(randmeandiff, breaks=50,
...
# One tailed
sum(randmeandiff<=diff.mean)/length(randmeandiff)

# Two tailed
sum(abs(randmeandiff)<=diff.mean)/length(randmeandiff)

```

### Using RITest

A development version of RItools allows for tests of sharp hypotheses for many different test statistics and randomization designs. Because it is a development version it does not have great documentation yet and has to be installed directly from github. For that reason, we recommend that you install it to a local library and not to your main R library. 
...
library(devtools)
ritoolsinstalled<-installed.packages(lib.loc="libraries")
if(!any(ritoolsinstalled[,"Package"]=="RItools")){
...
This replicates above with a simpler function:

```{r}

## Not blocked
thedesign<-simpleRandomSampler(z=dat$complete.ra,b=rep(1,nrow(dat)))

meandiffTZ <- function(ys, z) { mean(ys[!(!z)]) - mean(ys[!z]) }

set.seed(12345)
meandifftest<-RItest(y=dat$complete.obs, z=dat$complete.ra, test.stat=meandiffTZ, sampler = thedesign,samples=10000)
## FOr now, by default, reports the upper p.value
1-meandifftest
...
xb1<-xBalance(complete.ra ~ complete.obs, data=dat,report="all")
xb1

``` Notice, that in this case, you could have done the test much faster with
`xBalance` because the large sample size and simple outcome allows for a
Central Limit Theorem to operate.
...
plot(density(randmeandiff))
curve(dnorm(x,sd=sd(randmeandiff),mean=mean(randmeandiff)),add=TRUE,lty=2)
```
